...
# uploads them into a FullForwardFFICacheCuda, generates N tokens with greedy
# sampling, and parity-checks against the native forward path.
#
# This is the building block for the user's stated end-state:
# (1) full accelerated forward -- done here (FullForwardFFICacheCuda),
...
def generate_ffi(cache, prompt, n_new, t_seq, vocab)
  ...
  while step < n_new
    padded = pad_to_t(ids, t_seq)
    TinyNNCuda.upload_int_array(cache.sess, cache.t_token_ids, padded)
    TinyNNCuda.tnn_compute(cache.sess)
    logits = TinyNNCuda.download_row_major(cache.sess, cache.t_logits, t_seq, vocab)
    last    = ids.length - 1
    next_id = argmax_row(logits, last, vocab)
    ids.push(next_id)
    step = step + 1
