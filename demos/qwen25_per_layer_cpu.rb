# CPU half of the per-layer divergence test. Outputs first 16
# logit values for shallow N-layer forward passes. Pair with
# qwen25_per_layer_cuda.rb.
...
while ti < test_ns.length
  ...
  logits = SmolLM2KV.decode_step(kv, TOKEN, 0)

  print "N=" + n_layers.to_s + " cpu logits[0..7]:"
  i = 0
  while i < 8
  ...
  # Find argmax for top-line summary.
  ...
  puts "N=" + n_layers.to_s + " cpu argmax=" + best.to_s + " val=" + best_v.to_s
  ti = ti + 1
end
